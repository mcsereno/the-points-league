...
export async function POST(request:Request){
  ...
  if(body.status&&!["pending","approved","suspended","eliminated"].includes(body.status))return Response.json({error:"Invalid status."},{status:400});
  ...
  await recordAudit("member_updated","member",body.id,commissioner.member.email,{status:body.status??current.status,balance:body.balance??current.balance,startingBalance:body.startingBalance??current.startingBalance});
