...
import { syncLeagueOdds, syncNflPreseasonOdds } from "../../../lib/odds-sync";
...
export async function POST(request:Request){
  ...
  const payload=await request.json().catch(()=>null) as {league?:unknown;week?:unknown;scope?:unknown}|null;
  ...
  if(payload?.scope!=null&&payload.scope!=="nfl-preseason")return Response.json({error:"Refresh scope is not valid."},{status:400});
  if(payload?.scope==="nfl-preseason"&&payload.league&&payload.league!=="nfl")return Response.json({error:"NFL preseason refresh is only available for NFL."},{status:400});
  ...
  try{
    const preseason=payload?.scope==="nfl-preseason";
    const oddsResults=preseason?[await syncNflPreseasonOdds(true)]:[];
    if(!preseason)for(const league of leagues)oddsResults.push(await syncLeagueOdds(league,true,weekKey));
    const scoreResults=preseason?[]:await Promise.all(leagues.map(league=>syncLeagueScores(league,week)));
    ...
    const label=preseason?"NFL preseason":leagues.map(league=>league.toUpperCase()).join(" + ");
