...
export async function createOrUpdateMember(user:AuthenticatedUser){
  ...
  if(!existing){
    const usernameOwner=await env.DB.prepare("SELECT id FROM members WHERE display_name=? COLLATE NOCASE LIMIT 1").bind(username).first<{id:number}>();
    if(usernameOwner)throw new Error("That username is already taken. Choose another username before signing in.");
  }
